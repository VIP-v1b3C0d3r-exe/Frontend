...
const EVENT_IMAGES = {
  ...
  Workshops:
    "https://images.unsplash.com/photo-1531482615713-2afd69097998?w=120&q=80",
  Meetups:
...
const FilterPanel = ({
  ...
  useEffect(() => {
    ...
    if (!onFilterChange || Object.keys(categoryMap).length === 0) {
      return;
    }
  ...
  }, [activeCategory, categoryMap, locationFilter, dateFilter, onFilterChange]);
  ...
  const filtered = events.filter((event) => {
    ...
    const matchesCategory =
      !activeCategory ||
      (event.categoryIds &&
        event.categoryIds.includes(categoryMap[activeCategory]));
  ...
  const getCategoryImage = (event) => {
    ...
    const categoryName = Object.keys(categoryMap).find((name) =>
      event.categoryIds?.includes(categoryMap[name])
    );
  ...
  return (
    ...
    >
      ...
      <div className={styles.filterBtnsRow}>
        <FilterDropdown
          label="Time"
          activeCount={(dateFilter ? 1 : 0) + (timeOfDay ? 1 : 0)}
        >
            ...
            <div className={styles.ampmRow}>
              <button
                className={`${styles.ampmBtn} ${
                  timeOfDay === "AM" ? styles.ampmActive : ""
                }`}
              ...
              <button
                className={`${styles.ampmBtn} ${
                  timeOfDay === "PM" ? styles.ampmActive : ""
                }`}
          ...
          filtered.map((event) => (
            ...
            >
              <img
                ...
                onError={(e) => {
                  e.target.src = EVENT_IMAGES.Music;
                }}
              />
              ...
              <div className={styles.cardBody}>
                ...
                <p className={styles.cardMeta}>
                  ...
                  · {event.time} · {event.city || event.location}
                </p>
